...
func (h *Handler) loginProcess(tx *sqlx.Tx, userID int64, requestAt int64) (*User, []*UserLoginBonus, []*UserPresent, error) {
	...
	if err != nil {
		fmt.Println("obtain present: ", err)
		return nil, nil, nil, err
	}
	...
	*/
	...
	for _, np := range normalPresents {
		...
		/*
			...
			if err != nil {
				fmt.Println("insert user_present_all_received_history: ", err)
				return nil, err
			}
...
func (h *Handler) createUser(c echo.Context) error {
	...
	res, err := tx.Exec(query, user.ID, user.LastActivatedAt, user.RegisteredAt, user.LastGetRewardAt, user.CreatedAt, user.UpdatedAt)
	if err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) drawGacha(c echo.Context) error {
	...
	if err = h.getDB(userID).Get(gachaInfo, query, gachaID, requestAt); err != nil {
...
func (h *Handler) receivePresent(c echo.Context) error {
	...
	query, args, err := sqlx.In(query, requestAt, requestAt, ids)
	tx.Rebind(query)
